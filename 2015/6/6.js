...
const re = /(toggle|turn on|turn off){1} (\d{1,3}),(\d{1,3}) through (\d{1,3}),(\d{1,3})/
...
const amount = LIGHTS.map((a) => a.reduce((a, b) => a + b, 0)).reduce((a, b) => a + b, 0)
console.log(amount);
...
const amount2 = LIGHTS2.map((a) => a.reduce((a, b) => a + b, 0)).reduce((a, b) => a + b, 0)
console.log(amount2);
